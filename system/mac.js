...
serenade.global().command("clear line", async (api) => {
  await api.pressKey("backspace", ["command"]);
});

serenade.global().command("clear word", async (api) => {
  await api.pressKey("backspace", ["option"]);
});


serenade.global().command("skip right", async (api) => {
  await api.pressKey("right", ["option"]);
});

serenade.global().command("skip left", async (api) => {
